...
ユーザの支払いに対して、カウンセラーへの支払い額を確定することができないため、上記3つのChargeに対して、Connectアカウントの紐付ける方法は断念し、Unlaceの口座から、直接入金するようにしています。
![](/images/stripe-operation/counselor_payout.png)
カウンセラーの入金する際の金額によって、手数料が変わるため、Unlaceのシステム側で残高を管理するようにして、入金する際は、Connectアカウントへの送金と入金処理を一度に行うようにしています。
