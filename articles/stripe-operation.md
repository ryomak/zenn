...
僕自身、0->1のサービス開発を経験するのは、個人開発を除くとUnlaceが初めてだったので、ぶつかった壁が沢山ありました。。。
その中でも特に苦労したのは、決済基盤です。 経験したことがある領域とはいえ、既に完成されているものと1から作るものでは、難易度が段違いでした。

決済基盤について記事を書くにあたり、書きたい内容が多かったので、前半と後半に分けて記事を書きます。

前半は、Unlaceの全体の決済フローを見ていただきながら、要件に対してどのようにStripeを活用したのかをまとめます。
後半では、一番苦戦した、分析用途での決済基盤の設計について書く予定です。
Unlaceでは計算されたリスクを取り素早く行動することを重要視しているので、素早い事業の意思決定を行うためには、プロダクトを客観的なに把握できるデータが必要です。


今回の記事は、前半にあたります。
全体的なUnlaceにおけるStripeを利用した決済基盤の導入と実際に開発をする中でぶつかった壁とその対応事例をご紹介します。
...
大まかな設計と実装の説明をしましたが、ここから実際運用する中でぶつかった壁をご紹介します。
...
プランを購入は相談内容を回答してから、コンタクトが届くまでの間で行います。 なので仕様上、プラン購入とカウンセリングを始める間に、時間差が生じてしまいます。    
Unlaceではカウンセリングの期間をフルで利用してもらうために、サブスクリプションの開始はカウンセリング始めるタイミングにする必要がありました。

### 対応
Stripeのデフォルトはサブスクリプションの開始と決済は同じタイミングです。  
プラン購入からカウンセリングの開始までの時間が未確定のため、サブスクリプションを単純に作成するだけだと対応できなかったので、  

プラン購入はサブスクではなく、PaymentIntentでの単発購入にし、初回メッセージをしたタイミングから、Subscriptionを作る対応にしました。
ただそのままサブスクリプションを作成すると、作成したタイミングから請求が走るため、サブスクリプション作成時の`BillingCycleAnchor`を次の更新日時に指定し、更新日時から請求を開始するようにしました。
...
### 結論
StripeのサブスクリプションはPriceで指定している期間を元に走っているため、Schedule機能を使って短くはできますが、期間を先に伸ばすことはできません。  
...
- カウンセラーが自由に入金する金額を選択することができる

![](/images/stripe-operation/counselor_payout.png)
このため、Unlaceでは、上記画像の一番下のフローを選択しました。
Chargeに対して、Connectアカウントの紐付けは断念し、Unlaceの口座から、直接入金するようにしています。  
また、Connectアカウントの残高は用いずに、Unlaceのシステム側で残高を管理し、入金する際は、Connectアカウントへの送金と入金処理を一度に行うようにしています。
...
- どちらか1つの方式しか設定できない
- 設定の変更はいつでも可能だが、システムの反映に時間がかかることがあり、問い合わせの回答としては、入金の1週間前を目安に変更をしておくのがおすすめとのことでした。
...
カウンセラーの手動入金時のみ、入金方式を「手動入金」処理が終わった後に、「自動入金」に戻すと手段も検討しましたが、以下の理由により採用しませんでした。
- 手動入金が月末の自動入金と近い場合、自動入金が正しく月末に入金できないことがある
- 自動入金時に、Unlace側で管理しているカウンセラーの残高の同期に運用コストが発生する
入金開始から着金まで時間がかかるのは仕様としました。
...
今回は前半部分として、全体的なUnlaceにおけるStripe決済の設計と導入を事例としてご紹介しました。
