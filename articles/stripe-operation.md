...
その中でも特に苦労したのは、決済基盤です。
経験したことがある領域とはいえ、既に完成されているものと1から作るものでは、難易度が段違いでした。

決済基盤について記事を書くにあたり、書きたい内容が多かったので、前半と後半に分けて記事を書きます。

前半は、UnlaceにおけるStripeによる導入~運用までを予定しています。  
Unlaceの全体の決済フローを見ていただきながら、要件に対してどのようにStripeを活用したのかをまとめます。
...
しかし、Stripeのデフォルトはサブスクリプションの開始と決済は同じタイミングです。トライアルという仕組みはありますが、カウンセリング開始時刻が未確定のため、利用できませんでした。  
