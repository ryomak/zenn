...
決済基盤についての学びについて記事を書くにあたり、書きたい内容が多かったので、前半と後半に分けて記事を書きます。  
前半は、Unlaceのサービスの全体内容を見ていただきながら、Stripeを用いてどのように決済しているのかを書きます。
実装

後半では、一番苦戦した、分析用途での決済基盤の設計について書く予定です。
Unlaceでは計算されたリスクを取り素早く行動することを重要視しているので、素早い事業の意思決定を行うためには、プロダクトを客観的に把握する必要があります。 
